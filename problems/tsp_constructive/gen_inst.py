...
def generate_datasets():
    ...
    for problem_size in [20, 50, 100, 200]:
        n_instances = 100
        train_dataset = np.random.rand(n_instances, problem_size, 2)
        np.save(os.path.join(basepath, f'train{problem_size}_dataset.npy'), train_dataset)

    for problem_size in [20, 50, 100, 200]:
        n_instances = 64
        val_dataset = np.random.rand(n_instances, problem_size, 2)
        np.save(os.path.join(basepath, f'val{problem_size}_dataset.npy'), val_dataset)
