...
class MCTS_AHD:
    ...
    def expand(self, mcts, cur_node, nodes_set, option):
        if option == 's1':
            ...
            if len(path_set) == 1:
                # Even if path_set length is 1, still perform evaluation
    ...
    def find_and_remove_mcts_node(self, mcts_node, raw_info_to_find):
        # Recursively find and remove a node from the MCTS tree
        for child in mcts_node.children[:]:  # Iterate over a copy
            if child.raw_info['algorithm'] == raw_info_to_find['algorithm'] and child.raw_info['objective'] == raw_info_to_find['objective']:
                mcts_node.remove_child(child)
                # Also update children_info if it exists
                if hasattr(mcts_node, 'children_info'):
                    for info in mcts_node.children_info[:]:
                        if info['algorithm'] == raw_info_to_find['algorithm'] and info['objective'] == raw_info_to_find['objective']:
                            mcts_node.children_info.remove(info)
                return True
            if self.find_and_remove_mcts_node(child, raw_info_to_find):
                return True
        return False
    ...
    def run(self):
        ...
        while self.eval_times < self.fe_max:
            ...
            if iteration_counter % 100 == 0:
                print(f"\n--- Pruning population at iteration: {iteration_counter} (evals: {self.eval_times}) ---")
                current_pop_size = len(nodes_set)
                if current_pop_size > 1:  # Ensure there's something to prune
                    # Sort population, best first (lower objective is better)
                    nodes_set.sort(key=lambda x: x['objective'])
                    
                    num_to_keep = int(current_pop_size * 0.7)
                    
                    if num_to_keep < current_pop_size:
                        nodes_to_remove = nodes_set[num_to_keep:]
                        
                        print(f"Population size before pruning: {current_pop_size}")
                        print(f"Removing {len(nodes_to_remove)} worst-performing nodes.")
                        
                        # Remove from population
                        nodes_set = nodes_set[:num_to_keep]
                        
                        # Remove from MCTS tree to free up space
                        removed_count = 0
                        for node_info in nodes_to_remove:
                            if self.find_and_remove_mcts_node(mcts.root, node_info):
                                removed_count += 1
                        
                        print(f"Removed {removed_count} nodes from the MCTS tree.")
                        print(f"Population size after pruning: {len(nodes_set)}")
